...
class DQN(nn.Module):
    ...
    def get_second_action(self, x):
        with torch.no_grad():
            if len(x.shape) == 1:
                x = x.view(1, -1)
            q_v = self.forward(x)[0]
            q_v,ind = q_v.sort()
            return ind[-2]
...
class DQNAgent:
    ...
    def get_remain_action(self, o , env):
        print(env.current_state)
        for host_addr in env.network.sensitive_addresses:
            print(host_addr)
            # if not state.host_has_access(host_addr, AccessLevel.ROOT):
            #     return False
        return True
        return env.current_state[0]
    ...
    def run_eval_episode(self,
        ...
        while not done and not env_step_limit_reached:
            ...
            if r==2 or r==prev_r:
                eval_epsilon=1
                # a = self.get_remain_action(o, env)
            else:
                eval_epsilon=0.01
            ...
            o = next_o 
            if render and r>-4 and r!=-2:
                if r>50:
                    episode_return+=100
                else:
                    episode_return -= env.action_space.get_action(a).cost
                steps += 1
                print("\n" + line_break)
                print(f"Step {steps}")
                print(line_break)
                print(f"Action Performed = {env.action_space.get_action(a)}")
                print(f"total number of actions={env.action_space.get_number_actions()}")
                env.render(render_mode)
                print(f"Reward = {r}")
                print(f"Done = {done}")
